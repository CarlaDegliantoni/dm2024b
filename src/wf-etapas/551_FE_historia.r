...
if (envg$PARAM$lag4) {
  ...
  envg$OUTPUT$lag4$ncol_despues <- ncol(dataset)
  GrabarOutput()
  cat( "Fin lag4\n")
}
...
if (envg$PARAM$lag5) {
  ...
  envg$OUTPUT$lag5$ncol_antes <- ncol(dataset)
  ...
  envg$OUTPUT$lag5$ncol_despues <- ncol(dataset)
  GrabarOutput()
  cat( "Fin lag5\n")
}
